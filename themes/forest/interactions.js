...
export const INTERACTIONS = {
  ...
  'read-sign': {
    ...
    execute: (gameState, grid, x, y) => {
      const signText = grid[y][x].object?.config?.text || 'No text on this sign.';

      return {
        success: true,
        showSignDialog: true, // Special flag to show sign dialog
        signText: signText,
        modifyGrid: false
      };
    }
  ...
  'unlock-path-gate': {
    ...
    requirements: { tile: 'path-gate' },
    checkCustom: (gameState, tile, grid, x, y) => {
      // Parse sequence from config
      const sequenceJson = tile.object?.config?.sequence || '[]';
      let sequence;
      try {
      ...
      return checkPathSequence(gameState, sequence);
    },
    execute: (gameState, grid, x, y) => {
      const tile = grid[y][x];
      const message = tile.object?.config?.openMessage || 'The gate opens!';

      // Remove gate from object layer
      grid[y][x].object = null;
...
// Check if player has walked the correct path sequence
function checkPathSequence(gameState, sequence) {
  if (!sequence || sequence.length === 0) return false;

  const reachedLocations = gameState.reachedLocations || [];

  // Check each position was visited in the correct order
  // We search for the LAST occurrence of each tile to handle cases where
  // the player may have visited a path tile before starting the sequence
  let lastFoundIndex = -1;

  for (const pos of sequence) {
    const key = `${pos.x},${pos.y}`;

    // Find the FIRST occurrence of this tile after the previous tile
    let foundIndex = -1;
    for (let i = lastFoundIndex + 1; i < reachedLocations.length; i++) {
      if (reachedLocations[i] === key) {
        foundIndex = i;
        break; // Take the first occurrence after lastFoundIndex
      }
    }

    // Position not found after the previous position
    if (foundIndex === -1) {
      return false;
    }
